...
from avatar2 import Avatar, QemuTarget, GDBTarget

from . import InMemoryBufferPeripheral
from .peripherals import MemoryForwarderPeripheral
...
class ConvenientAvatar(Avatar):
    ...
    def add_in_memory_buffer_peripheral(
        ...
        mem_range = self.add_memory_range(
            address, size, name=name, emulate=InMemoryBufferPeripheral, permissions=permissions
        )
    ...
    def add_memory_forwarder_peripheral(self, address: int, size : int, va_machineDst: int, machineDst: GDBTarget, name: str = None):

        mem_range = self.add_memory_range(address, size, name=name, forwarded=True)

        peripheral: MemoryForwarderPeripheral = MemoryForwarderPeripheral(name=name, address=address, size=size, va_machineDst=va_machineDst, machineDst=machineDst)

        mem_range.forwarded_to = peripheral
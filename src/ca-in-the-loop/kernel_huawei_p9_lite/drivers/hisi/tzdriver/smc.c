...
#include "../smc_forwarder/smc_forwarder.h"
...
static noinline int smc_send(uint32_t cmd, phys_addr_t cmd_addr, uint32_t cmd_type, uint8_t wait)
{
	...
	//printk("\nTEETEST smc_send cmd_type: %llx", cmd_type);
...
static int smc_thread_fn(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		while (last_out != cmd_data->last_out) {
			...
			// getting smcs here will include agent smcs
			// agent smcs forwarding does work but there is no option to exec an agent SMC from an emulator
			// for now disabled - instead get returning smcs in smc_send_func()
			//smc_write_out(&cmd, true);
...
static void smc_work_func(struct kthread_work *work)
{
	...
	// check for incoming smcs or block if smcs are deactivated
	//smc_read_in();
	//smc_write_out(s_work->cmd, false);

	tc_smc_wakeup();

	//smc_write_out(s_work->cmd, true);
...
{
	// comment in if you want to log smcs to kernel logs
	/*printk("\nTEETEST cmd_type: %llx", cmd_type);
	printk("\nTEETEST SMC_SEND_FUNC_BEGIN", NULL);
	// print complete smc_cmd struct
	printk("\nTEETEST smc_cmd->uuid_phys: %llpx", cmd->uuid_phys);
	void *uuid = phys_to_virt(cmd->uuid_phys);
	printk("\nTEETEST virt_uuid: %p", uuid);
	print_hex_dump_bytes("\nTEETEST uuid: ", DUMP_PREFIX_NONE, uuid, 17);
	printk("\nTEETEST smc_cmd->cmd_id: %llx", cmd->cmd_id);
	printk("\nTEETEST smc_cmd->dev_file_id: %llx", cmd->dev_file_id);
	printk("\nTEETEST smc_cmd->context_id: %llx", cmd->context_id);
	printk("\nTEETEST smc_cmd->agent_id: %llx", cmd->agent_id);
	printk("\nTEETEST smc_cmd->operation_phys: %p", cmd->operation_phys);
	if(cmd->operation_phys)	{
		TC_NS_Operation *operation = phys_to_virt(cmd->operation_phys);
		if(operation){
			printk("\nTEETEST operation->paramTypes %llx", operation->paramTypes);
			int j = 0;
			int param_type = 0;
			while(j < 4) {
				param_type = TEEC_PARAM_TYPE_GET(operation->paramTypes, j);
				printk("\nTEETEST operation->param %x", j);
				if(param_type == TEEC_VALUE_OUTPUT || param_type == TEEC_VALUE_INPUT) {
					printk("\nTEETEST value a: %x", operation->params[j].value.a);
					printk("\nTEETEST value b: %x", operation->params[j].value.b);
				}
				if(param_type == TEEC_MEMREF_TEMP_INPUT || param_type == TEEC_MEMREF_TEMP_OUTPUT) {
					void *temp_buffer = phys_to_virt(operation->params[j].memref.buffer);
					printk("\nTEETEST buffer: %p", temp_buffer);
					print_hex_dump_bytes("\nTEETEST ", DUMP_PREFIX_NONE, temp_buffer, operation->params[j].memref.size);
					printk("\nTEETEST size: %x", operation->params[j].memref.size);
				}
				j = j + 1;
			}
	...
	printk("\nTEETEST smc_cmd->login_method: %llx", cmd->login_method);
	printk("\nTEETEST smc_cmd->login_data: %llx", cmd->login_data);
	printk("\nTEETEST smc_cmd->err_origin: %llx", cmd->err_origin);
	printk("\nTEETEST smc_cmd->ret_val: %llx", cmd->ret_val);
	printk("\nTEETEST smc_cmd->event_nr: %llx", cmd->event_nr);
	printk("\nTEETEST smc_cmd->remap: %llx", cmd->remap);
	printk("\nTEETEST smc_cmd->uid: %llx", cmd->uid);
	printk("\nTEETEST smc_cmd->started: %llx\n", cmd->started);
	printk("\nTEETEST SMC_SEND_FUNC_END", NULL);*/

	// check for incoming smcs or block if smcs are deactivated
	smc_read_in();

	printk("\nSMCTEST before smc_send_func addr of sent smc_cmd: %p", cmd);
	smc_write_out(cmd, false);
	...
	printk("\nSMCTEST smc_thread_fn addr of ret smc_cmd: %p", &cmd);
	// write out returning smc will exclude agent smcs
	smc_write_out(cmd, true);
...
{
	printk("\nAGENTTEST before posting smc cmd addr of sent smc_cmd: %p", cmd);
	printk("\nAGENTTEST before posting smc cmd agent_id of sent smc_cmd: %x", cmd->agent_id);
	printk("\nAGENTTEST before posting smc cmd event_nr of sent smc_cmd: %p", cmd->event_nr);
	...
	printk("\nAGENTTEST after posting smc cmd event_nr of sent smc_cmd: %p", cmd->event_nr);

		   ...
		   uint8_t flags)
{
	...
	// print bytes of smc_cmd struct
	//print_hex_dump_bytes("\nTEETEST SMC_CMD STRUCT: ", DUMP_PREFIX_NONE, smc_cmd, sizeof(TC_NS_SMC_CMD));

	// print bytes of operation
	//if(operation)	{
	//	print_hex_dump_bytes("\nTEETEST OPERATION STRUCT: ", DUMP_PREFIX_NONE, operation, sizeof(TC_NS_Operation));
	//}
	

	// print complete smc_cmd struct
	/*printk("\nTEETEST smc_cmd->uuid_phys: %llpx", smc_cmd->uuid_phys);
	print_hex_dump_bytes("\nTEETEST uuid: ", DUMP_PREFIX_NONE, uuid, 17);
	printk("\nTEETEST smc_cmd->cmd_id: %llx", smc_cmd->cmd_id);
	printk("\nTEETEST smc_cmd->dev_file_id: %llx", smc_cmd->dev_file_id);
	printk("\nTEETEST smc_cmd->context_id: %llx", smc_cmd->context_id);
	printk("\nTEETEST smc_cmd->agent_id: %llx", smc_cmd->agent_id);
	printk("\nTEETEST smc_cmd->operation_phys: %p", smc_cmd->operation_phys);
	if(operation){
		printk("\nTEETEST operation->paramTypes %llx", operation->paramTypes);
		int j = 0;
		int param_type = 0;
		while(j < 4) {
			param_type = TEEC_PARAM_TYPE_GET(client_context->paramTypes, j);
			...
			if(param_type == TEEC_MEMREF_TEMP_INPUT || param_type == TEEC_MEMREF_TEMP_OUTPUT) {
				printk("\nTEETEST buffer: %p", local_temp_buffer[j].temp_buffer);
				print_hex_dump_bytes("\nTEETEST ", DUMP_PREFIX_NONE, local_temp_buffer[j].temp_buffer, local_temp_buffer[j].size);
				printk("\nTEETEST size: %x", local_temp_buffer[j].size);
			}
			j = j + 1;
		}
		
	}
	printk("\nTEETEST smc_cmd->login_method: %llx", smc_cmd->login_method);
	printk("\nTEETEST smc_cmd->login_data: %llx", smc_cmd->login_data);
	printk("\nTEETEST smc_cmd->err_origin: %llx", smc_cmd->err_origin);
	printk("\nTEETEST smc_cmd->ret_val: %llx", smc_cmd->ret_val);
	printk("\nTEETEST smc_cmd->event_nr: %llx", smc_cmd->event_nr);
	printk("\nTEETEST smc_cmd->remap: %llx", smc_cmd->remap);
	printk("\nTEETEST smc_cmd->uid: %llx", smc_cmd->uid);
	printk("\nTEETEST smc_cmd->started: %llx\n", smc_cmd->started);*/

	// dumping smcs here will NOT get every smc! e.g. register_agent will not go through here
	//smc_read_in();
	//smc_write_out(smc_cmd, operation, local_temp_buffer, uuid, false);

	/* send smc to secure world */
	tee_ret = TC_NS_SMC(smc_cmd, flags);

	//smc_write_out(smc_cmd, operation, local_temp_buffer, uuid, true);

	/*printk("\nTEETEST-----RETURN VALUES-----", NULL);
	printk("\nTEETEST smc_cmd->uuid_phys: %llpx", smc_cmd->uuid_phys);
	print_hex_dump_bytes("\nTEETEST uuid: ", DUMP_PREFIX_NONE, uuid, 17);
	printk("\nTEETEST smc_cmd->cmd_id: %llx", smc_cmd->cmd_id);
	printk("\nTEETEST smc_cmd->dev_file_id: %llx", smc_cmd->dev_file_id);
